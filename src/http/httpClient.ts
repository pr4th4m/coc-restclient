...
import { Uri, workspace, Document, WorkspaceConfiguration } from "coc.nvim";
...
export class HttpClient {
  ...
  public constructor(config: WorkspaceConfiguration, doc: Document) {
    ...
    return new HttpResponse(
      ...
      new HttpRequest(
        ...
        Buffer.isBuffer(requestBody)
          ? this.convertBufferToStream(requestBody)
          : requestBody,
        httpRequest.rawBody,
        httpRequest.name
      )
  ...
  private async prepareOptions(
  ...
  ): Promise<got.GotBodyOptions<null>> {
    ...
    const options: got.GotBodyOptions<null> = {
      ...
      cookieJar: this._settings.rememberCookiesForSubsequentRequests
        ? new CookieJar(this.cookieStore, { rejectPublicSuffixes: false })
        : undefined,
      retry: 0,
      hooks: {
        afterResponse: [],
    ...
    if (
      ...
      !HttpClient.ignoreProxy(
        httpRequest.url,
        this._settings.excludeHostsForProxy
      )
    ) {
    ...
    if (options.cookieJar) {
      const {
        getCookieString: originalGetCookieString,
        setCookie: originalSetCookie,
      } = options.cookieJar;

      function _setCookie(
        cookieOrString: Cookie | string,
        currentUrl: string,
        opts: CookieJar.SetCookieOptions,
        cb: SetCookieCallback
      ): void;
      function _setCookie(
        cookieOrString: Cookie | string,
        currentUrl: string,
        cb: SetCookieCallbackWithoutOptions
      ): void;
      function _setCookie(
        cookieOrString: Cookie | string,
        currentUrl: string,
        opts: CookieJar.SetCookieOptions | SetCookieCallbackWithoutOptions,
        cb?: SetCookieCallback
      ): void {
      ...
      if (hasHeader(options.headers!, "cookie")) {
        ...
        function _getCookieString(
          currentUrl: string,
          opts: CookieJar.GetCookiesOptions,
          cb: GetCookieStringCallback
        ): void;
        function _getCookieString(
          currentUrl: string,
          cb: GetCookieStringCallback
        ...
        function _getCookieString(
          currentUrl: string,
          opts: CookieJar.GetCookiesOptions | GetCookieStringCallback,
          cb?: GetCookieStringCallback
        ): void {
          if (opts instanceof Function) {
            cb = opts;
          ...
          originalGetCookieString.call(
            ...
            (err, cookies) => {
              ...
              cb!(
                null,
                [cookies, getHeader(options.headers!, "cookie")]
                  .filter(Boolean)
                  .join("; ")
              );
            }
  ...
  private getRequestCertificate(requestUrl: string): Certificate | null {
    ...
    if (
      !host ||
      !(host in this._settings.get<HostCertificates>("certificates", {}))
    ) {
      return null;
    }
    ...
    const {
      ...
      pfx: pfxPath,
      passphrase,
      // } = this._settings.hostCertificates[host];
    } = this._settings.get<HostCertificates>("certificates", {})[host];
  ...
  private resolveCertificate(
    ...
    const rootPath = getWorkspaceRootPath(this.document);
    let absolutePath = "";
    if (rootPath) {
      absolutePath = path.join(
        Uri.parse(rootPath).fsPath,
    ...
    const currentFilePath = getCurrentHttpFileName(this.document);
    if (!currentFilePath) {
      return undefined;
    }
