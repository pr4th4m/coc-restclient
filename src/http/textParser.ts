...
export class HttpTextParser {
  private readonly responseStatusLineRegex = /^\s*HTTP\/[\d.]+/;

  public getRequest(rawText: string, position: number): SelectedRequest | null {
    let selectedText = this.getDelimitedText(rawText, position);
  ...
  public isFileVariableDefinitionLine(line: string): boolean {
    return Constants.FileVariableDefinitionRegex.test(line);
  }

  public isResponseStatusLine(line: string): boolean {
  ...
  public getRequestVariableDefinitionName(text: string): string | undefined {
    const matched = text.match(Constants.RequestVariableDefinitionRegex);
    return matched?.[1];
  }

  public hasNoteComment(text: string): boolean {
    return Constants.NoteCommentRegex.test(text);
  }

  public hasWarningComment(text: string): boolean {
    return Constants.WarnCommentRegex.test(text);
  }
  ...
  private getDelimiterRows(lines: string[]): number[] {
